...
UNIRATE_BASE_API = "https://api.unirateapi.com/api/convert?api_key={api_key}&amount=1&from={coin}&to={base}"
...
async def get_coin_price(coin: str, base: str = "USD") -> str:
    ...
    data = await _request(UNIRATE_BASE_API.format(api_key=api_key, coin=coin, base=base))
    if not data:
